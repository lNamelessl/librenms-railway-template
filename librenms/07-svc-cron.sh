...
# Upstream disables cron when SIDECAR_DISPATCHER=1 because the sidecar
# topology splits scheduler and dispatcher into separate containers.
# On Railway, one volume per service forbids sidecars, so this image runs
# BOTH the cron scheduler AND the librenms dispatcher inside the main
# container (same combination as the upstream compose stack). We therefore
# only skip cron for the other sidecar roles, never for the dispatcher.
